import 'package:tractian_challenge/features/assets/data/models/asset_model.dart';
import 'package:tractian_challenge/features/assets/data/models/location_model.dart';
import 'package:tractian_challenge/features/assets/domain/entities/asset_entity.dart';
import 'package:tractian_challenge/features/assets/domain/entities/filtro_params.dart';
import 'package:tractian_challenge/features/assets/domain/entities/location_entity.dart';

const tCompanyId = '1';

final tSemFiltro = FiltroParams(nome: '');

final tComFiltro = FiltroParams(nome: 'Asset', isCritico: true);

Map<String, dynamic> tSemDadosResponse = {
  "data": [],
};

Map<String, dynamic> tBadResponse = {
  "data": "lista",
};

Map<String, dynamic> tLocationResponse = {
  "data": [
    {
      "name": "CHARCOAL STORAGE SECTOR",
      "id": "656a07b3f2d4a1001e2144bf",
      "parentId": "65674204664c41001e91ecb4"
    },
    {
      "name": "Empty Machine house",
      "id": "656733611f4664001f295dd0",
      "parentId": null
    },
    {
      "name": "Machinery house",
      "id": "656733b1664c41001e91d9ed",
      "parentId": null
    },
    {
      "name": "PRODUCTION AREA - RAW MATERIAL",
      "id": "65674204664c41001e91ecb4",
      "parentId": null
    }
  ]
};

Map<String, dynamic> tAssetResponse = {
  "data": [
    {
      "name": "CONVEYOR BELT ASSEMBLY",
      "id": "656a07bbf2d4a1001e2144c2",
      "locationId": "656a07b3f2d4a1001e2144bf",
      "parentId": null,
      "sensorType": null,
      "status": null
    },
    {
      "name": "Fan - External",
      "id": "656734821f4664001f296973",
      "locationId": null,
      "parentId": null,
      "sensorType": "energy",
      "status": "operating"
    },
    {
      "name": "Fan H12D",
      "id": "656734448eb037001e474a62",
      "locationId": "656733b1664c41001e91d9ed",
      "parentId": null,
      "sensorType": null,
      "status": null
    },
    {
      "name": "MOTOR RT COAL AF01",
      "id": "656a07cdc50ec9001e84167b",
      "locationId": null,
      "parentId": "656a07c3f2d4a1001e2144c5",
      "sensorType": "vibration",
      "status": "operating"
    },
    {
      "name": "MOTOR TC01 COAL UNLOADING AF02",
      "id": "656a07c3f2d4a1001e2144c5",
      "locationId": null,
      "parentId": "656a07bbf2d4a1001e2144c2",
      "sensorType": null,
      "status": null
    },
    {
      "name": "Motor H12D- Stage 1",
      "id": "6567340c1f4664001f29622e",
      "locationId": null,
      "parentId": "656734968eb037001e474d5a",
      "sensorType": "vibration",
      "status": "alert"
    },
    {
      "name": "Motor H12D-Stage 2",
      "id": "6567340c664c41001e91dceb",
      "locationId": null,
      "parentId": "656734968eb037001e474d5a",
      "sensorType": "vibration",
      "status": "alert"
    },
    {
      "name": "Motor H12D-Stage 3",
      "id": "656733921f4664001f295e9b",
      "locationId": null,
      "parentId": "656734968eb037001e474d5a",
      "sensorType": "vibration",
      "status": "alert"
    },
    {
      "name": "Motors H12D",
      "id": "656734968eb037001e474d5a",
      "locationId": "656733b1664c41001e91d9ed",
      "parentId": null,
      "sensorType": null,
      "status": null
    }
  ]
};

final tLocationList = (tLocationResponse["data"] as List)
    .map((location) => LocationModel.fromMap(location))
    .toList() as List<LocationEntity>;

final tAssetList = (tAssetResponse["data"] as List)
    .map((asset) => AssetModel.fromMap(asset))
    .toList() as List<AssetEntity>;
